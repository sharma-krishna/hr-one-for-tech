  ...
  render() {
    const getWorkTime = day => {
      if (day["punchIn"] !== null && day["punchOut"] !== null) {
        return (
          parseInt(day["punchOut"].split(":")[0] * 60) +
          parseInt(day["punchOut"].split(":")[1]) -
          (parseInt(day["punchIn"].split(":")[0] * 60) +
            parseInt(day["punchIn"].split(":")[1]))
        );
    ...
    const addAttendence = () => {
      this.props.attendance.forEach(day => {
        ...
        if (employeeWorkTime === Infinity) {
          ...
          employeeWorkTime >= HALF_DAY_TIME &&
          employeeWorkTime < FULL_DAY_TIME
        ) {
          attendenceMarkObject = {
            ...attendenceMarkObject,
            [day["date"]]: halfDay
    ...
    const handlePress = day => {
      console.log(day);
      const obj = this.props.attendance.filter(
        att => day.dateString == att.date
      );
      console.log(obj);
      this.setState(state => {
        state.active = true;
      ...
      <View style={styles.container}>
        ...
        {this.state.active ? (
          <DetailBox
            ...
            flag={true}
            changeActive={this.changeActiveState}
          />
        ) : null}
      </View>
    );
...
export default compose(withConnect)(Attendance);
